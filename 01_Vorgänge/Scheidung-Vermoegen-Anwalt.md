# Scheidung / Vermögen – Vorgang & Anwalt (Stand 11.07.2026)

⚠️ **Kein Rechtsrat.** Ava bündelt Fakten, rechnet grob, bereitet aufs Anwaltsgespräch vor. Verbindliche Aussagen nur vom Fachanwalt Familienrecht.

## Ausgangslage (von Martin bestätigt)
- Verheiratet mit **Jana Esters**, **getrennt lebend**, noch nicht geschieden.
- **Kein Ehevertrag** → gesetzlicher Güterstand **Zugewinngemeinschaft**.
- Kinder: **Lea** & **Luka** (minderjährig) → Kindesunterhalt + Sorge/Umgang relevant.
- Jana war **Angestellte** bei ME Digital (nicht bestätigt Gesellschafterin).

## WICHTIGE KORREKTUR zum Denkmodell „alles 50:50"
Bei Zugewinngemeinschaft wird **NICHT** das Vermögen halbiert. Jeder behält sein Eigentum.
Ausgeglichen wird nur der **Zugewinn** = (Endvermögen − Anfangsvermögen) je Ehegatte.
Wer mehr Zugewinn hatte, zahlt die **Hälfte der Differenz** als Geldausgleich (Zugewinnausgleich).
→ Entscheidend sind Stichtage: **Heirat** (Anfangsvermögen) und **Zustellung Scheidungsantrag** (Endvermögen).
Firmenanteile bleiben bei dir; nur ihr **Wertzuwachs während der Ehe** fließt in die Rechnung.

## Vermögenswerte (aus Drive + Angaben)
| Position | Detail | Eigentümer | offen |
|---|---|---|---|
| **Wohnung Mallorca** | Kaufpreis **1,4 Mio € cash** | ? auf wen im escritura | Eigentümer, Kaufdatum, aktueller Wert |
| **Haus Karl-Rüsing-Str. 2** (Meerbusch) | Grundbuch/Statik/Baukosten im Drive; Pool, Terrasse, PV | ? | Eigentümeranteile, Wert, Restschuld |
| **SoD ScreenOnDemand** | 15 % Beteiligung, Bewertung 60 Mio → Wert **9 Mio €** | Martin | Finanziert via Darlehen 3 Mio € → **Nettowert 6 Mio €** |
| **ME Digital Marketing GmbH** | Holding | Martin | 100 %? Wert? |
| **ME Retail GmbH** | 50/50 Martin + Jana | Beide je 50 % | Wert? |
| **ME Immobilienverwaltung GmbH** | 50/50 Martin + Jana, Immobilien ~1 Mio € drin | Beide je 50 % | Restschulden auf Immobilien? |
| **MUVN** | Seed-Beteiligung via ME Digital (Notar 14.07.) | Martin (mittelbar) | Investsumme |
| **Autos** | G63 (gestohlen→Allianz), BMW X5, Tesla Model S | ? | Halter/Eigentümer, Werte |
| **Konten** | ING (Giro + Gemeinschaftskonto mit Jana), N26, Commerzbank | tw. gemeinsam | Salden Stichtag |
| **Geldanlage** | Oskar/Baader (evtl. Kinder) | ? | Depotwerte |

## Bestätigte Eckdaten (20.07.2026)
- **Heiratsdatum:** 14.09.2014
- **Trennungsdatum:** 01.07.2025 → Trennungsjahr abgelaufen, Scheidungsantrag ab sofort möglich
- **Anfangsvermögen Martin bei Heirat:** ~0 €
- **Mallorca:** auf beiden Namen (Martin + Jana) → je 50 %
- **Meerbusch:** auf beiden Namen + Grundschuld vorhanden
- **SoD:** 15 % Beteiligung, letzte Bewertung 60 Mio € → Martins Anteil = **9 Mio €**
- **SoD gegründet 2017 → NACH der Heirat** → vollständig zugewinnrelevant

## Kernergebnis Zugewinnausgleich (Stand 22.07.2026)
**Martin zahlt Jana ~3 Mio €** (Grundlage: SoD-Nettowert 6 Mio €, alle 50/50-Positionen heben sich auf). Spielraum ±200–400 k je nach Endwerten.

## Noch offen
- Meerbusch: aktueller Marktwert + Restschuld Grundschuld
- ME Digital GmbH: Wert, 100 % Martin?
- Janas Anfangsvermögen bei Heirat
- Unterhaltsvereinbarung mit Jana vorhanden?

## Vorgehensplan
1. **Anwalt:** Fachanwalt Familienrecht Düsseldorf, spezialisiert auf **Unternehmer-/Vermögensscheidungen** (Firmenbewertung, internationale Immobilie ES). → Ava sucht 2–3 Kandidaten.
2. **Bestandsaufnahme Stichtage:** Anfangsvermögen (Heirat) vs. Endvermögen sauber dokumentieren.
3. **Unternehmensbewertung:** Bewertung SoD/Holding zum Stichtag (Anwalt + WP/Steuerberater).
4. **Mallorca:** spanischen Notarvertrag/Registerauszug (nota simple) besorgen – liegt nicht im Drive.
5. **Liquiditätsplanung:** Zugewinnausgleich ist Geldschuld → prüfen, woher (ohne Firmenanteile zu verkaufen).
6. **Kinder:** Kindesunterhalt (Düsseldorfer Tabelle), Sorge-/Umgangsregelung.

## Anwalt-Kandidaten (recherchiert 20.07.2026)

| # | Kanzlei | Schwerpunkt | Adresse | Telefon | Kontakt |
|---|---|---|---|---|---|
| 1 | **Schlun & Elseven** | Unternehmer-Scheidung, HNW, internationale Immobilien – am passendsten für deinen Fall | Düsseldorf | – | se-legal.de/online-anfrage |
| 2 | **Benjamin Deutscher** | Fachanwalt Familienrecht, 5,0/5 ⭐ auf anwalt.de, Einzelkanzlei | Hüttenstr. 1, 40215 Düsseldorf | 0211 868 1456 | mail@ra-deutscher.de |
| 3 | **Tondorf Böhm Leber** | Etabliert seit 1967, Fachanwalt Familienrecht (Niklas Böhm) | Scheibenstraße 47, 40479 Düsseldorf | 0211 864630 | kanzlei@tondorfboehm.de |

**Empfehlung:** Schlun & Elseven zuerst kontaktieren – die haben explizit Unternehmer-/Vermögensscheidung und internationale Immobilien als Schwerpunkt, was exakt zu deinem Fall passt (Holding, SoD, Mallorca-Wohnung).

- **Status:** noch keiner mandatiert.

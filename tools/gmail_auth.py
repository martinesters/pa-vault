#!/usr/bin/env python3
"""Manueller (fern-tauglicher) OAuth-Flow für Gmail.
  gmail_auth.py url <token_dir>
  gmail_auth.py exchange <token_dir> "<redirect_url_or_code>"
Speichert Credentials als JSON unter <token_dir>/gmail_token.json.
Scopes: gmail.modify (lesen, Entwürfe, senden) + calendar (mitgenommen, schadet nicht).
"""
import sys, os, json
# Google gibt oft zusaetzliche Scopes (openid, ...) zurueck -> Warnung unterdruecken
...
def cmd_url(token_dir):
    ...
    with open(os.path.join(token_dir, ".gmail_auth_state.json"), "w") as f:
...
def cmd_exchange(token_dir, redirect_arg):
    ...
    flow = make_flow()
    sp = os.path.join(token_dir, ".gmail_auth_state.json")
    ...
    c = flow.credentials
    out = os.path.join(token_dir, "gmail_token.json")
